...
<sql:param value="${name}" />
	...
	<!--  
<c:url var="addAction" value="/airplane/add" ></c:url>

<form:form action="${addAction}" commandName="airplane">
<table>
	<c:if test="${!empty airplane.aircrafttype}">
	<tr>
		<td>
		...
		<td>
			<form:input path="airplane_id" readonly="true" size="8"  disabled="true" />
			<form:hidden path="airplane_id" />
		</td> 
	</tr>
	</c:if>
	<tr>
		<td>
			<form:label path="aircrafttype">
				<spring:message text="Aircrafttype"/>
			</form:label>
		</td>
		<td>
			<form:input path="aircrafttype" />
		</td> 
	</tr>
	<tr>
		<td>
			<form:label path="departure">
				<spring:message text="Departure"/>
			</form:label>
		</td>
		<td>
	...
	<tr>
		<td>
			<form:label path="arrival">
				<spring:message text="Arrival"/>
			</form:label>
		</td>
		<td>
	...
	<tr>
		<td>
			<form:label path="fromm">
				<spring:message text="From"/>
			</form:label>
		</td>
		<td>
			<form:input path="fromm" />
		</td>
	</tr>
	<tr>
		<td>
	...
	<tr>
		<td>
			<form:label path="delay">
				<spring:message text="Delay"/>
			</form:label>
		</td>
		<td>
			<form:input path="delay" />
		</td>
	</tr>
	<tr>
		<td>
			<form:label path="seats">
				<spring:message text="Seats"/>
			</form:label>
		</td>
		<td>
	...
	<tr>
		<td>
			<form:label path="seatsr">
				<spring:message text="Seatsr"/>
			</form:label>
		</td>
		<td>
			<form:input path="seatsr" />
		</td>
	</tr>
		<td colspan="2">
			<c:if test="${!empty airplane.aircrafttype}">
...
-->
		...
		<div class=MainBodyFirstBar>
			...
			<div class=search style="position: relative;left:80px;top:60px">
				...
				<table id="tableairplane" style="position: relative;left:-83px">
					...
					<c:forEach items="${listAllAirplanes}" var="airplane">
						<tr id="trr">
							<td>${airplane.airplane_id}</td>
							<td>${airplane.aircrafttype}</td>
							<td>${airplane.departure}</td>
							<td>${airplane.arrival}</td>
							<td>${airplane.fromm}</td>
							<td>${airplane.too}</td>
							<td>${airplane.delay}</td>
							<td>${airplane.seats}</td>
							<td>${airplane.seatsr}</td>
							<td><a
								href="<c:url value='airplane/add/${clientId}/${airplane.airplane_id}' />"
								style="text-decoration: none"
								onclick="myFunction(${airplane.airplane_id})">Add</a></td>
	...
	<sql:setDataSource var="snapshot" driver="com.mysql.jdbc.Driver"
		url="jdbc:mysql://localhost:3306/lic" user="root" password="root" />
	<table class="abbrevTable" style="position:absolute;left:930px;top:72px">
		<tr>
			<th>City</th>
			<th>Abbrev</th>
		</tr>
